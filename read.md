...
pipeline {
    ...
    tools {
        jdk 'JAVA-17'          // Make sure Java 17 is installed in Jenkins
        maven 'MAVEN-HOME'    // Make sure Maven is configured in Jenkins
    }

    environment {
        REPO_URL = 'https://github.com/srinidhikasetty/maven_webapp.git'
        PROJECT_DIR = 'maven_webapp'
        TOMCAT_PATH = 'C:\\Tomcat\\webapps' // Adjust if Tomcat is in a different location
    }
    ...
    stages {
        ...
        stage('Deploy to Tomcat') {
            steps {
                ...
                bat "rmdir /s /q ${TOMCAT_PATH}\\${PROJECT_DIR}"
                // Copy new WAR file
                bat "copy ${PROJECT_DIR}\\target\\*.war ${TOMCAT_PATH}\\"
            }
        }

        stage('Clean Workspace') {
            steps {
                bat "rmdir /s /q ${PROJECT_DIR}"
            }
        }
    }
    ...
    post {
        ...
        success {
            echo 'SUCCESS: Web app deployed successfully!'
        }
        failure {
            echo 'FAILURE: Check the Jenkins logs for details.'
        }
    }
}
